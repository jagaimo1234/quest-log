...
function QuestActionDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md border-none shadow-2xl bg-white/90 backdrop-blur-sm dark:bg-zinc-900/90">
        <DialogHeader>
        ...
        <div className="grid grid-cols-2 gap-4 p-4">
          <Button
            variant="outline"
            onClick={handleAbort}
            className="h-24 flex flex-col gap-2 border-2 border-muted hover:border-destructive/50 hover:bg-destructive/10 hover:text-destructive transition-all"
          >
            <XCircle className="w-8 h-8" />
            <span className="font-bold">Abort (Fail)</span>
          </Button>
          <Button
            variant="outline"
            onClick={handleDelete}
            className="h-24 flex flex-col gap-2 border-2 border-muted hover:border-red-500/50 hover:bg-red-500/10 hover:text-red-500 transition-all"
          >
...
function TodayItem({
  ...
  export default function Home() {
    ...
    const [actionQuest, setActionQuest] = useState<any>(null);
        ...
        <main className="layout-container py-8 mx-auto">
                      ...
                      {todayQuests.map(q => (
                        ...
                        >
                          <TodayItem
                            ...
                            onLongPress={(q) => setActionQuest(q)}
                            onDragStart={(e) => {
                  ...
                  {(() => {
                    ...
                    return (
                      <TodayItem
                        ...
                        onLongPress={() => { }} // No long press in drag preview
                      />
                    );
                  })()}
                </div>
          ...
          {actionQuest && (
            <QuestActionDialog
              quest={actionQuest}
              open={!!actionQuest}
              onOpenChange={(open) => !open && setActionQuest(null)}
              onUpdated={refreshAll}
            />
          )}
        </main>
